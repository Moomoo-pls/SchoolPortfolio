...
{    
	protected int[] directCost = new int[] {7, 0, 2, 0};
	protected int[] leastCost = new int[] {7, 999, 2, 0};
    ...
    public Entity3()
    {
		...
		for(int i = 0; i < 4; i++)
		{
			if(isDirect[i] == 1)
			{
				Packet toSend = new Packet(3, i, leastCost);
		...
		for(int i = 0; i < 4; i++)
		{
			if(i != 3)
			{
				...
				if(tempCost < distanceTable[i][p.getSource()])
				{
					...
					System.out.println("Updated Node 3's distance table.");
					printDT();
				}
